import java.util.*;
import java.math.*;



/**
 * This program uses big number to computer the odds of winning the grand prize in a lottery
 * @author Administrator
 *
 */
public class BigIntegerTest {

	public static void main(String[] args) {
		Scanner in = new Scanner(System.in);
		
		System.out.println("How many numbers do you need to draw? ");
		int k = in.nextInt();
		
		System.out.println("What is the highest nubmer you can draw? ");
		int n = in.nextInt();
		
		/**
		 * compute binomial coefficient n*(n-1)*(n-2)*....*(n-k+1)/(1*2*3*...*k)
		 */
		
		BigInteger lotterOdds = BigInteger.valueOf(1);
		
		for(int i = 1; i <= k; i++){
			lotterOdds = lotterOdds.multiply(lotterOdds.valueOf(n - i + 1)).divide(BigInteger.valueOf(i));
//		System.out.println("You odds are 1 in " + lotterOdds + ". Good Luck!");
		}
		
		System.out.println(lotterOdds);
	}

}
